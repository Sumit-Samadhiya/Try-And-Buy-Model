...
export default function DisplayAllBrand()
 {
   ...
   const [brandName,setBrandName]=useState('')
    ...
    function listAllBrand() {
        ...
        return (
          <MaterialTable
            title={<TitleComponent title={'List Brand'} listicon='' />}
            columns={[
              { title: 'id', field: 'id' },
      ...
      const showBrandDialog=()=>{
        return(<Dialog open={open} fullWidth={true} maxWidth={"sm"}>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
                <TextField value={brandName} error={formError.brandname} helperText={formError.brandname} onFocus={()=>handleError(false,'brandname')} onChange={(event)=>setBrandName(event.target.value)} fullWidth label="Brand Name"></TextField>
            </Grid>
