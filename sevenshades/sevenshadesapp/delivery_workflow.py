...
from .models import DeliveryRider, DeliveryAssignment, DeliveryBatch, TryOrder, TryOrderItem, FinalOrder
...
def generate_batches(rider_id):
    ...
    for order_id in candidates:
        ...
        if FinalOrder.objects.filter(try_order=order).exists():
            continue
        groups[(order.postcode, order.delivery_slot, timezone.localdate(order.created_at))].append(order)
    result = []
